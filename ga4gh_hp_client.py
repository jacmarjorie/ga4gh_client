...
def GASearchVariantsRequest(query):
    ...
    for key, val in chrs.iteritems():
        ...
        oncotator_build(request)

def variants_search(this_request):
    r = requests.post('http://localhost:8000/v0.5.1/variants/search', data=json.dumps(this_request), headers={'content-type':'application/json'})
    return json.loads(r.text)

def vs_recurse(this_request):
    r_json = variants_search(this_request)
    ...
    while nextPageToken != None:
        ...
        new_response = variants_search(new_request)
        responses = responses + new_response['variants']
        nextPageToken = new_response['nextPageToken']
    return responses

def oncotator_build(request):
    variants = vs_recurse(request)
    tsv_string = ''
    build = 'hg19'
...
if __name__ == "__main__":
    ...
    print url
    print args.query
    GASearchVariantsRequest(args.query)
